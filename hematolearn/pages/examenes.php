...
$examenes = $db->query('SELECT * FROM examenes WHERE activo=1 ORDER BY id ASC')->fetchAll();

$stmtS = $db->prepare('
    SELECT examen_id, puntaje, completado, terminado_en
    FROM intentos_examen
    WHERE usuario_id = ? AND completado = 1
    ORDER BY terminado_en DESC
');
$stmtS->execute([$uid]);
$intentos = [];
foreach ($stmtS->fetchAll() as $row) {
    $intentos[$row['examen_id']] = $row;
}

$completados  = count($intentos);
$promedio     = $completados > 0 ? round(array_sum(array_column($intentos, 'puntaje')) / $completados, 1) : 0;
...
<head>
    ...
    <title>HematoLearn | Exámenes</title>
...
<div class="modal-acceso-overlay" id="modalAcceso">
  ...
  <path fill-rule="evenodd" d="M1.5 6a2.25 2.25 0 0 1 2.25-2.25h16.5A2.25 2.25 0 0 1 22.5 6v12a2.25 2.25 0 0 1-2.25 2.25H3.75A2.25 2.25 0 0 1 1.5 18V6ZM3 16.06V18c0 .414.336.75.75.75h16.5A.75.75 0 0 0 21 18v-1.94l-2.69-2.689a1.5 1.5 0 0 0-2.12 0l-.88.879.97.97a.75.75 0 1 1-1.06 1.06l-5.16-5.159a1.5 1.5 0 0 0-2.12 0L3 16.061Zm10.125-7.81a1.125 1.125 0 1 1 2.25 0 1.125 1.125 0 0 1-2.25 0Z" clip-rule="evenodd" />
                ...
                <li><a href="examenes.php" class="sidebar-link activo" title="Exámenes"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
...
<br>Registro</a>
        ...
        <main class="contenido">
            <div style="margin-bottom:0.5rem;">
                <h1 class="galeria-titulo">Exámenes y Evaluaciones</h1>
                <p class="galeria-subtitulo">Pon a prueba tus conocimientos sobre células sanguíneas</p>
            </div>
            ...
            <div class="examenes-stats">
                <div class="stat-card">
                    ...
                    <div>
                        <div class="stat-numero"><?= count($examenes) ?></div>
            ...
            <h2 class="examenes-titulo">Exámenes disponibles</h2>
            <div class="examenes-grid">
                <?php foreach ($examenes as $ex):
                    $intento     = $intentos[$ex['id']] ?? null;
                    $yaHecho     = $intento !== null;
                    $difClass    = $ex['dificultad'];
                    $difLabel    = ['basico'=>'Básico','intermedio'=>'Intermedio','avanzado'=>'Avanzado'][$difClass] ?? $difClass;
                ?>
                <div class="examen-card">
                    <div class="examen-header">
                        <p class="examen-nombre"><?= htmlspecialchars($ex['titulo']) ?></p>
                        <span class="dificultad-badge <?= $difClass ?>"><?= $difLabel ?></span>
                    </div>
                    <p class="examen-desc"><?= htmlspecialchars($ex['descripcion']) ?></p>
                    <div class="examen-meta">
                        <span>📝 10 preguntas</span>
                        <span>⏱ <?= $ex['tiempo_min'] ?> minutos</span>
                    </div>
                    <?php if ($yaHecho): ?>
                    <div style="display:flex; align-items:center; gap:0.5rem; margin-bottom:0.25rem;">
                        <span class="tag tag-morado">✓ Completado</span>
                        <span style="font-size:0.78rem; color:var(--gris-texto);">
                            Puntaje: <strong><?= $intento['puntaje'] ?>%</strong>
                        </span>
                    </div>
                    <?php endif; ?>
                    <a href="examen.php?id=<?= $ex['id'] ?>" class="examen-btn">
                        <?= $yaHecho ? 'Repetir examen →' : 'Iniciar examen →' ?>
                    </a>
                </div>
                <?php endforeach; ?>
            </div>
